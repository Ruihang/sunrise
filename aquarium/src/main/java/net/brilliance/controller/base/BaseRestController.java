...
public abstract class BaseRestController<T> {
  ...
  @Inject
	 ...
	 * this.fetchBusinessObjectByCode(code); } catch (Exception e) { log.error(e.getMessage()); } return fetchedObject;
	 * }
	 * 
	 ...
	 * 
	 * responseEntity = ResponseEntity.created(projectUri).build(); } catch (Exception e) { log.error(e.getMessage());
