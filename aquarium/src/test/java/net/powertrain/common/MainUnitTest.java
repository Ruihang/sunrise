	...
	*//**
	...
	public static void main(String[] args) {
		try {
			...
			String fileName = "D:/user-data/1. Thuá»‘c tÃ¢n dÆ°á»£c/1. Danh má»¥c thuá»‘c tÃ¢n dÆ°á»£c.xlsx";
			...
			testClasses();
			String encodedUrl = URLEncoder.encode("04A/GPÄ�C1-Ä�N", "UTF-8");
	...
	private static boolean isDepartment(String codeValue){
		final String regularExpression = "^ChÆ°Æ¡ng*?$";
		return CommonUtility.regularExpressionCompiled(regularExpression, codeValue);
	}

	private boolean isCatalog(String codeValue){
		final String regularExpression = "^PHáº¦N*?$";
		return CommonUtility.regularExpressionCompiled(regularExpression, codeValue);
	}

	protected static void testExternalDataHelper(){
		final String regExpDepartment = "ChÆ°Æ¡ng";
	...
	protected static void testMarshallProducts(){
		try {
			String resourceFileName = "F:/Downloads/DANH-Má»¤C-HÃ€NG-HÃ“A-XUáº¤T-KHáº¨U-NHáº¬P-KHáº¨U-VIá»†T-NAM-2015.xlsx";
			...
			for (List<String> stringElements :((List<List<String>>)bucket.get(sheet))){
				if (CommonUtility.isNotEmpty(stringElements.get(0)) && CommonUtility.isEmpty(stringElements.get(1)) && CommonUtility.isEmpty(stringElements.get(2))){
					if (stringElements.get(0).startsWith("PHáº¦N ")){
					...
					} else if (stringElements.get(0).startsWith("ChÆ°Æ¡ng ")){
